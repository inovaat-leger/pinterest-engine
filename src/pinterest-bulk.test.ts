...
const config: CampaignConfig = {
  ...
  pinterestBulkSchedule: { startDate: "2026-08-18", timezone: "America/Chicago", dailyTimes: ["08:00", "11:00", "14:00", "17:00", "20:00"], pinsPerDay: 5 },
};
...
test("Pinterest bulk CSV uses exact official headers and planned non-reserve Pins", () => {
  ...
  assert.equal(records[0]["Media URL"], "https://travel.stampdup.com/pins/philippines/concept-10.png");
  assert.equal(new URL(records[0].Link).searchParams.get("utm_content"), "pin_002");
});

test("Pinterest schedule assigns five local slots per day chronologically and converts to UTC", () => {
  assert.equal(localTimeToPinterestUtc("2026-08-18", "08:00", "America/Chicago"), "2026-08-18T13:00:00");
  ...
  assert.deepEqual(rows.slice(0, 6).map((row) => row["Publish date"]), [
    "2026-08-18T13:00:00", "2026-08-18T16:00:00", "2026-08-18T19:00:00", "2026-08-18T22:00:00", "2026-08-19T01:00:00", "2026-08-19T13:00:00",
  ]);
...
test("Pinterest CSV escapes commas and quotation marks", () => {
  const selected = pins();
  selected[1] = { ...selected[1], title: "A \"quoted\", useful title", description: "Details, choices, and \"checks\"." };
