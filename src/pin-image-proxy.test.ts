...
test("approved Philippines Pin IDs retain their exact registry artwork and canonical titles", () => {
  ...
  assert.ok(identities.slice(24).every((identity) => identity.filename.startsWith(`pin-${identity.pinId.slice(4)}-`) && /-v[2-9][0-9]*\.png$/.test(identity.filename) && identity.driveFileId && !identity.localPath));
});
